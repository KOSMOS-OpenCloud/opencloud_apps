...
function instrumentVueRouter(router, options, startNavigationSpanFn) {
  ...
  router.beforeEach((to, _from, ...rest) => {
    ...
    for (const key of Object.keys(to.params)) {
      attributes$1[attributes.URL_PATH_PARAMETER_KEY.replace("<key>", key)] = to.params[key];
      attributes$1[attributes.PARAMS_KEY.replace("<key>", key)] = to.params[key];
    }
    for (const key of Object.keys(to.query)) {
      const value = to.query[key];
      if (value) {
        attributes$1[`query.${key}`] = value;
      }
    }
    ...
    if (transactionSource === "route") {
      attributes$1[attributes.URL_TEMPLATE] = spanName;
    }
    ...
    if (options.instrumentNavigation && !activePageLoadSpan) {
      startNavigationSpanFn(
        ...
        browser.getAbsoluteUrl(to.fullPath ?? to.path)
      );
    }
    if (isLegacyRouter) {
      const next = rest[0];
